     ...
     function veTongQuan(root) {
            ...
            (function () {
                     ...
                     the.innerHTML =
                                ...
                                (thieuSerTuan ? theHtml('Reno', '<span class="bc-mo-chu">—</span>', 'chưa có số theo TUẦN cho phạm vi của bạn — xem ở Báo cáo tháng', '', '', '') : theHtml('Reno', fInt(ser.tong.RENO) + ' <small>máy</small>', serT ? kt.nhan + ': ' + fInt(serT.tong.RENO) : '', chip(serT ? pct(ser.tong.RENO, serT.tong.RENO) : null), spark(ser12.map(function (s) { return s.tong.RENO; })), kenhCoSo.map(function (c) { var v = sK(ser, c, 'RENO'), vt = serT ? sK(serT, c, 'RENO') : null; return '<div><i class="bc-cham" style="background:' + mau(c) + '"></i><b>' + c + '</b><span>' + fInt(v) + '</span>' + chip(pct(v, vt)) + '</div>'; }).join(''))) +
                                (thieuSerTuan ? theHtml('Find', '<span class="bc-mo-chu">—</span>', 'chưa có số theo TUẦN cho phạm vi của bạn — xem ở Báo cáo tháng', '', '', '') : theHtml('Find', fInt(ser.tong.FIND) + ' <small>máy</small>', serT ? kt.nhan + ': ' + fInt(serT.tong.FIND) : '', chip(serT ? pct(ser.tong.FIND, serT.tong.FIND) : null), spark(ser12.map(function (s) { return s.tong.FIND; })), kenhCoSo.map(function (c) { var v = sK(ser, c, 'FIND'), vt = serT ? sK(serT, c, 'FIND') : null; return '<div><i class="bc-cham" style="background:' + mau(c) + '"></i><b>' + c + '</b><span>' + fInt(v) + '</span>' + chip(pct(v, vt)) + '</div>'; }).join(''))) +
